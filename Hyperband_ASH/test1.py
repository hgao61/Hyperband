...
print('gpus looks like {}'.format(gpus))
with concurrent.futures.ThreadPoolExecutor(len(gpus)) as executor:
    #results = [x for x in executor.map(fit, gpus)]
    results1 =  executor.map(fit, gpus[0])
    results2 =  executor.map(fit, gpus[1])
    results3 =  executor.map(fit, gpus[2])
    
print('results:{} {} {}'.format(results1,results2,results3))
