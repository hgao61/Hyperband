...
import operator
from get_params_vgg16 import get_params_vgg16
from get_params_mlp import get_params_mlp
try:
    ...
    def __init_workers(self):
        ...
        for gpuid in self._gpuids:
            if args.network == 'CNN':
                ...
                self._workers.append(MLPWorker(gpuid, self._queue))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--network", type=str, help="MLP or CNN")
    parser.add_argument("--mode", type=str, help="run random_search for benchmark,by default Hyperband")
    ...
    bracket_range = reversed( range( s_max + 1 ))
    if args.mode == 'random_search':
        bracket_range = range(1)
    ...
    for s in bracket_range:
        ...
        if args.network == 'CNN':
            T = [ get_params_vgg16() for i in range( n )]
        elif args.network == 'MLP':
            T = [ get_params_mlp() for i in range( n )]
        print ("s={}".format(s))
